function app_sorting_decelerationTest_3DOF(mu)
%% Introduction
% This function reads in the simulink model data obtained from deceleration
% test for the Simulink 3DOF vehicle model and processes it to ...
...
% MPC_Controller_deceTest in the cotroller_update_Test
% Load the file generated from simulink model based on 'mu' input

if mu == 0.7
    load('deceResults3DOF_mu0p7.mat');
elseif mu == 1
    load('deceResults3DOF_mu1.mat');
elseif mu == 1.5
    load('deceResults3DOF_mu1p5.mat');
else 
...
%% Exclude the acceleration data and obtain deceleratios
A = diff(vel);
I = find(A < 0);
vel = vel(I);
T = T(I);
...
% csvwrite('inc/dynamics/vehicles/Simulink3DOF/decelerationTable.csv',data);
